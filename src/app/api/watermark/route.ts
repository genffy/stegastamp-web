...
async function handleEncoder(filePath: string, secret: string, select='0,0,400,400') {
  ...
  try {
    ...
    const { stdout, stderr } = await promiseExec(`python3 web_cli.py --type encode --image ${filePath} --secret ${secret} --select '${select}'`, {
      signal,
    });
    clearTimer()
    data = stdout
...
async function handleDecoder(filePath: string) {
  ...
  try {
    const { signal, clearTimer } = abortSignal()
    const { stdout, stderr } = await promiseExec(`python3 web_cli.py --type decode --image ${filePath}`, {
      signal,
    });
    clearTimer()
    console.log(stdout, stderr)
